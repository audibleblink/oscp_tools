...
def mssqlEnum(ip_address, port):
    ...
    MSSQLSCAN = (
	 "nmap -vv -sV -Pn -p %s --script=ms-sql-info,ms-sql-config,ms-sql-dump-hashes "
	 "--script-args=mssql.instance-port=1433,smsql.username-sa,mssql.password-sa "
	 "-oX results/exam/nmap/%s_mssql.xml %s" 
...
def nmapScan(ip_address):
   ...
   serv_dict = {}
   TCPSCAN = "nmap -vv -Pn -A -sC -sS -T4 -p- \
	 -oN '%s/%s.nmap' \
